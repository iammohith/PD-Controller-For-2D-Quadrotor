...
- **`libs/`**:  
   ...
   - **`sys_params.m`**:  
     Sets system parameters, such as the quadrotor's mass $m$, gravitational acceleration $g$, arm lenght $l$, moment of inertia about the x-axis $I_xx$, minimum thrust input $u_{min}$ and maximum thrust input $u_{max}$. 
